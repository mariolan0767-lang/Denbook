...
function toggleAuthMode(mode) {
  ...
  showRegisterButton.className = registerActive ? "primary" : "ghost";
}
...
function setStateFromPayload(payload) {
  ...
  state.recaptchaSiteKey = payload.recaptchaSiteKey || null;
}
...
function renderRecaptcha() {
  if (!state.recaptchaSiteKey || !window.grecaptcha || state.recaptchaWidgetId !== null) return;
  recaptchaSlot.classList.remove("hidden");
  state.recaptchaWidgetId = window.grecaptcha.render("recaptcha-slot", {
...
async function handleRegister(name, email, password) {
  const payload = await api("/api/register", {
    method: "POST",
    body: JSON.stringify({ name, email, password, recaptchaToken: getRecaptchaToken() }),
  });
  setStateFromPayload(payload);
  closeAuth();
  renderAll();
