...
function securityHeaders() {
  return {
    ...
    "Content-Security-Policy":
      "default-src 'self'; img-src 'self' data: https:; style-src 'self' 'unsafe-inline' https://fonts.googleapis.com; font-src 'self' https://fonts.gstatic.com; script-src 'self' https://www.google.com/recaptcha/ https://www.gstatic.com/recaptcha/; frame-src https://www.google.com/recaptcha/; connect-src 'self' https://www.google.com/recaptcha/;",
  };
}
...
function bootstrap(db, request) {
  ...
  return {
    ...
    registrationRequiresEmailCode: false,
...
async function handleApi(db, request, response, pathname) {
  ...
  if (request.method === "POST" && pathname === "/api/register") {
    ...
    const email = sanitizeText(body.email, 120).toLowerCase();
    const password = String(body.password || "");
    const recaptchaToken = sanitizeText(body.recaptchaToken, 5000);
    ...
    const captchaOk = await verifyRecaptcha(recaptchaToken, request.socket.remoteAddress || "");
    if (!captchaOk) {
      sendJson(response, 400, { error: "reCAPTCHA dogrulamasi basarisiz." });
      return;
    }
    ...
    logAction(
      ...
      role === "admin" ? "Ilk kayit admin oldu" : "Yeni kayit"
    );
    sendJson(
      response,
      200,
      bootstrap(db, { headers: { cookie: `${SESSION_COOKIE}=${session.token}` } }),
      sendSessionCookies(response, session)
    );
    return;
  }
